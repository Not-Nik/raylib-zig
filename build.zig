...
pub fn build(b: *std.Build) !void {
    ...
    const raylib_gui = rl.gui.getModule(b, target, optimize);
    ...
    const raylib_gui_test = b.addTest(.{
        .root_source_file = b.path("lib/raygui.zig"),
        .target = target,
        .optimize = optimize,
    });
    raylib_gui_test.root_module.addImport("raylib-zig", raylib);
    ...
    test_step.dependOn(&raylib_gui_test.step);
    ...
    for (examples) |ex| {
        if (target.query.os_tag == .emscripten) {
            ...
            exe_lib.root_module.addImport("raylib-gui", raylib_gui);
